...
bool MaintainModule::Init()
{
	...
	m_pPubKernel = (CPubKernel*)pPluginManager->FindModule<IPubKernel>();
	m_pPerformanceCount = pPluginManager->FindModule<PerformanceCountModule>();

	mCommandFunMap["export_obj_list"] = std::bind(&MaintainModule::ExportOBJList, this, std::placeholders::_1);
	mCommandFunMap["export_obj"] = std::bind(&MaintainModule::ExportOBJ, this, std::placeholders::_1);
	mCommandFunMap["export_obj_guild"] = std::bind(&MaintainModule::ExportOBJGuild, this, std::placeholders::_1);
	mCommandFunMap["export_performance"] = std::bind(&MaintainModule::ExportPerformanceCount, this, std::placeholders::_1);
...
bool MaintainModule::AfterInit()
{
	...
	//执行命令
	m_pMaintainNetServer->AddReceiveCallBack(OuterMsg::RUN_NET_COMMAND, this, (NetMsgFun)&MaintainModule::OnCommandMsg);
...
bool MaintainModule::GetObjBeatList(const REQSelectObj& req, ACKGetObjBeatList& ack)
{
	...
	if (nullptr == pObj)
	{
		QLOG_WARING << __FUNCTION__ << " Obj == NULL:" << uid;
		return false;
	}
...
void MaintainModule::OnCommandMsg(const NFSOCK nSockIndex, const int nMsgID, const char* msg,
	const uint32_t nLen)
{
	...
	{
		m_pMaintainNetServer->SendMsg(OuterMsg::RUN_NET_COMMAND, "command does not exist.", nSockIndex);
		return;
	}

	std::string& reply = it->second(vec);
	m_pMaintainNetServer->SendMsg(OuterMsg::RUN_NET_COMMAND, reply, nSockIndex);
}
...
std::string MaintainModule::ExportOBJList(std::vector<std::string>& arg)
{
	...
	if (arg.size() < 2)
	{
		outFile.open("ExportOBJList_Pub.txt", std::ios::trunc);
	}
	else
	{
...
std::string MaintainModule::ExportOBJ(std::vector<std::string>& arg)
{
	...
	NFGUID guid = m_pPubKernel->GetPubGuid(arg[1]);
	if (guid.IsNull())
	{
		guid = arg[1];
	}
	...
	if (PBConvert::ToPBObject(m_pKernelModule, guid, &xObjPack))
	{
		...
		{
			outFile.open("ExportOBJ_Pub.txt", std::ios::trunc);
		}
...
std::string MaintainModule::ExportOBJGuild(std::vector<std::string>& arg)
{
	if (arg.size() < 2)
	{
		return "Parameter error.";
	...
	NFGUID guid = m_pPubKernel->GetGuildGuid(arg[1]);
	if (guid.IsNull())
	{
		guid = arg[1];
	}
	...
	if (PBConvert::ToPBObject(m_pKernelModule, guid, &xObjPack))
	{
		...
		{
			outFile.open("ExportOBJ_Guild.txt", std::ios::trunc);
		}
...
std::string MaintainModule::ExportPerformanceCount(std::vector<std::string>& arg)
{
	std::string out;
	m_pPerformanceCount->ToString(out);
