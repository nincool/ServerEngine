...
private:
	...
	void OnCommandMsg(const NFSOCK nSockIndex, const int nMsgID, const char* msg,
		const uint32_t nLen);

	std::string ExportOBJList(std::vector<std::string>& arg);
	std::string ExportOBJ(std::vector<std::string>& arg);
	std::string ExportOBJGuild(std::vector<std::string>& arg);
...
private:
	...
	CPubKernel* m_pPubKernel = nullptr;
...
#endif //__H_MaintainModule_H__#pragma once
