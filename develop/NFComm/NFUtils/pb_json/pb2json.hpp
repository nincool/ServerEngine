...
public:
    static bool Json2Message(const Json& json, ProtobufMsg& message, bool str2enum = false);
    static void Message2Json(const ProtobufMsg& message, Json& json, bool enum2str = false);

protected:
    static bool Json2RepeatedMessage(const Json& json, ProtobufMsg& message, const ProtobufFieldDescriptor* field,
...
void Pb2Json::Message2Json(const ProtobufMsg& message, Json& json, bool enum2str) {
	auto descriptor = message.GetDescriptor();
	auto reflection = message.GetReflection();
	if (nullptr == descriptor || nullptr == reflection) return;
